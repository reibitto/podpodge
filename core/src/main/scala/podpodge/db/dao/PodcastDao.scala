...
object PodcastDao extends SqlDao {
  ...
  def get(id: PodcastId): ZIO[Has[Connection] with Blocking, SQLException, Option[Podcast.Model]] =
    ctx.run {
  ...
  def create(podcast: Podcast.Insert): ZIO[Has[Connection] with Blocking, SQLException, Podcast[PodcastId]] =
    ctx.run {
  ...
  ): ZIO[Has[Connection] with Blocking, SQLException, List[Podcast[PodcastId]]]                             =
    ctx.run {
  ...
  def updateImage(id: PodcastId, s: Option[String]): ZIO[Has[Connection] with Blocking, SQLException, Long] =
    ctx.run {
      query[Podcast.Model].filter(_.id == lift(id)).update(_.image -> lift(s))
    }

  def delete(id: PodcastId): ZIO[Has[Connection] with Blocking, SQLException, Long] =
    ctx.run {
